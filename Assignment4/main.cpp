...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points, float t) 
{
    ...
    {
        for(int i=0;i<n-1;i++) res.push_back(control_points[i]*(1-t)+control_points[i+1]*t);
         ...
         for(int i = -1; i <= 1; i++){
            for(int j = -1; j <= 1; j++){//遍历9个像素
                if(point.y + j > 700 || point.y + j < 0 || point.x + i > 700 || point.x + i < 0)//不处理越界像素
                    continue;
                double ratio = 1 - sqrt(2)*sqrt(pow(point.y - int(point.y + j) - 0.5, 2) + pow(point.x - int(point.x + i) - 0.5, 2)) / 3;//计算ratio
